function [emdSTEM] = STEMtiltsGPU_QYN9(atoms,cellDim,xTiltArray,numFP)
...
s = single((2*pi/lambda/E0)*(m*c^2+e*E0)/(2*m*c^2+e*E0));
...
qxa = gpuArray(single(qxa));
qya = gpuArray(single(qya));
...
propArray = gpuArray(zeros(Nx,Ny,Ntilt(1),Ntilt(2),'single'));
for a0 = 1:Ntilt(1)
    xTilt = xTiltArray(a0);
    for a1 = 1:Ntilt(2)
        yTilt = yTiltArray(a1);
...
potLookup = zeros(length(xvec),length(yvec),length(atomTypes),'single');
uLookup = zeros(length(atomTypes),1,'single');
...
% Generate potentials
potAll = zeros(Nx,Ny,subSlice,'single');
...
trans = gpuArray(exp(1i*s*potAll));
if f_plotPot == 1
    potSum = sum(potAll,3);
end
...
emdSTEM.data = gpuArray(zeros( ...
    Nx/2,Ny/2,...
    length(thickness),...
    length(defocusArray),Ntilt(1),Ntilt(2),'single'));
emdSTEM.intData = zeros(thicknessArray(end),2);
psi = gpuArray(zeros(Nx,Ny,length(xp),length(yp),length(defocusArray),Ntilt(1),Ntilt(2),'single'));
for a0 = 1:numFP
...
emdSTEM.data = gather(emdSTEM.data / numFP / length(xp) / length(yp));
